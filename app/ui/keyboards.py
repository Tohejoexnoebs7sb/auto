...
def profile_admin_kb(profile_id):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"💾 فاصله بک‌آپ: {get_profile_backup_interval(profile_id)}", callback_data=f"setbackupinterval_{profile_id}", style="primary"),
         InlineKeyboardButton(f"⏰ کرون: {prof.get('schedule_cron') or 'خالی'}", callback_data=f"setcron_{profile_id}", style="primary")],
