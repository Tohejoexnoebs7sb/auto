...
def profile_admin_kb(profile_id):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⚙️ مدیریت پروتکل‌ها", callback_data=f"proto_menu_{profile_id}", style="primary"),
         InlineKeyboardButton("🧾 حالت نمایش هدر", callback_data=f"header_modes_{profile_id}", style="primary")],
        [InlineKeyboardButton("🗓 زمان‌بندی پیشرفته", callback_data=f"advanced_schedule_{profile_id}", style="primary")],
...
def general_settings_kb():
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(
            f"⚡ هسته Ping سراسری: {'Check-Host' if get_ping_engine_mode()==0 else ('Core / Relay Delay' if get_ping_engine_mode()==1 else 'Core + Relay Delay + Check-Host')}",
